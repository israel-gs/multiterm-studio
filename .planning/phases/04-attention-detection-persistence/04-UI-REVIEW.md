...
- `"No folder selected"` (`App.tsx:70`) is a status description, not a guide. A user who dismisses the native folder picker ends up reading "No folder selected" with a "Pick a folder" button below, but there is no explanation of _why_ they need to pick a folder or what the app does. Score impact: -1 from a 4.
