...
const ATTENTION_PATTERN = /(\? |[(\[](y\/n|Y\/N|y\/N|N\/y)[)\]]|do you want|press enter to continue|password:|continue\? )/i
...
function handleAttentionEvent(win: BrowserWindow, sessionId: string, panelTitle: string, snippet: string): void {
  if (!win.isFocused()) {
...
const ATTENTION_PATTERN = /(\? $|\(y\/n\)|\[y\/n\]|\(Y\/n\)|\[Y\/n\]|\(y\/N\)|\[y\/N\]|do you want|password:|press enter to continue|confirm\?)/i
```
