...
| Session persistence across reboots    | "Make panels survive machine restart" is a natural ask once per-project persistence is known.         | True session persistence requires daemonized PTY servers (like iTerm2's session restoration). Dramatically increases complexity, especially cross-platform.              | Per-project layout metadata (titles, colors, pane arrangement) persists. Shell session content does not — this is v1 scope. |
...
_Feature research for: Multi-terminal tiling desktop app (Electron / multiterm-studio)_
